...
public interface SmestajRepository extends JpaRepository<Smestaj, Long> {
	...
	@Query(value="SELECT DISTINCT SMESTAJ.VERSION, SMESTAJ.ID, SMESTAJ.ADRESA, SMESTAJ.BROJ_OCENA, SMESTAJ.DRZAVA, SMESTAJ.GMAP_URL, SMESTAJ.GRAD, SMESTAJ.NAZIV, SMESTAJ.OPIS, SMESTAJ.REJTING, SMESTAJ.TIP_ID, SMESTAJ.VLASNIK_ID"
			+ " FROM SMESTAJ, SMESTAJ_SOBE, SOBA "
			+ "WHERE SMESTAJ.ID=SMESTAJ_SOBE.SMESTAJ_ID AND SOBA.ID=SMESTAJ_SOBE.SOBE_ID AND "
			+ "UPPER(SMESTAJ.NAZIV)=UPPER(?1) AND SOBA.BROJ_LEZAJA>=?2 AND ("
			+ "(SMESTAJ.ZVEZDICE=?3)OR(SMESTAJ.ZVEZDICE=?4)OR(SMESTAJ.ZVEZDICE=?5)OR(SMESTAJ.ZVEZDICE=?6)OR(SMESTAJ.ZVEZDICE=?7)OR(SMESTAJ.ZVEZDICE=?8)"
			+ ")"
			, nativeQuery = true)
