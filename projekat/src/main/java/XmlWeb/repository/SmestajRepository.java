...
public interface SmestajRepository extends JpaRepository<Smestaj, Long> {
	...
	@Query(value="SELECT DISTINCT SMESTAJ.VERSION, SMESTAJ.ID, SMESTAJ.ADRESA, SMESTAJ.BROJ_OCENA, SMESTAJ.DRZAVA, SMESTAJ.GMAP_URL, SMESTAJ.GRAD, SMESTAJ.NAZIV, SMESTAJ.OPIS, SMESTAJ.REJTING, SMESTAJ.KATEGORIJA_ID, SMESTAJ.TIP_ID, SMESTAJ.VLASNIK_ID "
			...
			+ "("
			+ "(UPPER(SMESTAJ.NAZIV)=UPPER(?1) or UPPER(?1)='')AND "
			+ "(UPPER(SMESTAJ.ADRESA)=UPPER(?5)or UPPER(?5)='') AND "
			+ "(UPPER(SMESTAJ.GRAD)=UPPER(?3)or UPPER(?3)='') AND "
			+ "(UPPER(SMESTAJ.DRZAVA)=UPPER(?4)or UPPER(?4)='')"
			+ ") AND SOBA.BROJ_LEZAJA>=?2"
			, nativeQuery = true)
	List<Smestaj> findByNameAndSobaNumberSeats(String name, int size, String city, String drzava, String addres);
	
	@Query(value="SELECT DISTINCT SMESTAJ.VERSION, SMESTAJ.ID, SMESTAJ.ADRESA, SMESTAJ.BROJ_OCENA, SMESTAJ.DRZAVA, SMESTAJ.GMAP_URL, SMESTAJ.GRAD, SMESTAJ.NAZIV, SMESTAJ.OPIS, SMESTAJ.REJTING, SMESTAJ.KATEGORIJA_ID, SMESTAJ.TIP_ID, SMESTAJ.VLASNIK_ID "
			+ " FROM SMESTAJ, SMESTAJ_SOBE, SOBA "
			+ "WHERE SMESTAJ.ID=SMESTAJ_SOBE.SMESTAJ_ID AND SOBA.ID=SMESTAJ_SOBE.SOBE_ID AND "
			+ "(UPPER(SMESTAJ.ADRESA)=UPPER(?5) AND UPPER(SMESTAJ.GRAD)=UPPER(?3) AND UPPER(SMESTAJ.DRZAVA)=UPPER(?4) AND UPPER(SMESTAJ.NAZIV)=UPPER(?1)) AND SOBA.BROJ_LEZAJA>=?2"
			, nativeQuery = true)
	List<Smestaj> findByNameAndSobaNumberSeatsAndCategory(String name, int size);
